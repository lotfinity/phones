# d5e137da-3c67-417e-a87b-397e19fc0f57 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `instagram_ocr_ops.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `instagram_ocr_ops.html`
...
- A single self-contained `instagram_ocr_ops.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `instagram_ocr_ops.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
## Entry points
- `instagram_ocr_ops.html`
- `instagram-ocr-ops.html`
...
1. Inspect `instagram_ocr_ops.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
